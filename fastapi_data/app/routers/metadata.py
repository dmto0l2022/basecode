...
api_base_url = '/dmtool/fastapi_data/internal/metadata/'
...
async def get_dropdown_valuepairs(variable_in : str, session: AsyncSession = Depends(get_session)):
    result = await session.execute(select(Dropdown_valuepair).where(Dropdown_valuepair.variable==variable_in))
