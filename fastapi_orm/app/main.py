...
@app.get(
    "/apiorm/authlibuser/exists/{email}", response_model=User_authlib_count_Pydantic, responses={404: {"model": HTTPNotFoundError}}
)
async def get_authlibuser_count(email: str):
    user_count = await Users_authlib.filter(email=email).count()
    #return Status(message=f"user count >> {user_count}")
    return user_count
...
@app.delete("/apiorm/authlibuser/{email}", response_model=Status, responses={404: {"model": HTTPNotFoundError}})
